//! Parser for annotations within an Rustviz code block body.
...
impl Hash for RustvizAnnotations {
  fn hash<H: std::hash::Hasher>(&self, _state: &mut H) {
    // HashMaps aren't hashable, and we can ignore annotations when hashing
    // anyway since they don't change the result of an Rustviz computation.
  }
}

pub fn parse_annotations(code: &str) -> Result<(String, RustvizAnnotations)> {
  ...
  let mut annots = RustvizAnnotations::default();
  let mut idx = 0;
...
let y = 2;
  ...
  assert_eq!(annot, RustvizAnnotations {
